...
public class KianaTitleScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(new ClickableTextWidget(i, j, 70, 20, Text.literal("Worlds"), button -> this.client.setScreen(new SelectWorldScreen(this))));
        this.addDrawableChild(new ClickableTextWidget(i += 74, j, 70, 20, Text.literal("Servers"), button -> this.client.setScreen(new MultiplayerScreen(this))));
        this.addDrawableChild(new ClickableTextWidget(i += 74, j, 70, 20, Text.literal("Options"), button -> this.client.setScreen(new OptionsScreen(this, this.client.options))));
        if (modMenuInstalled) this.addDrawableChild(ModMenuCompat.getModsButton(this, i += 74, j));
        this.addDrawableChild(new ClickableTextWidget(i += 74, j, 70, 20, Text.literal("Quit"), button -> this.client.scheduleStop()));
